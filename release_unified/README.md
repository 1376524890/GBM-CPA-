...
# 2. 完整性检查
python release_unified/check_gbm_final_h5ad.py --h5ad GBM_NIPS_Ready.h5ad

# 3. 加载示例
python release_unified/load_gbm_example.py --h5ad GBM_NIPS_Ready.h5ad

# 4. 对 counts-space 预测重新评估（log1p 空间）
python release_unified/reevaluate_counts_predictions_log1p.py --h5ad GBM_NIPS_Ready.h5ad
```
...
| 文件 | 用途 |
|------|------|
| `check_gbm_final_h5ad.py` | 验证 h5ad 完整性（42+ 检查项） |
| `load_gbm_example.py` | 最小加载示例，支持命令行参数 |
| `evaluate_gbm_predictions_protocol.md` | 统一评估接入协议 |
| `GBM_数据集交接文档.md` | 面向其他组员的完整交接文档 |
| `GBM_QC检查报告.md` | 数据质量检查报告 |
| `GBM_dataset_manifest.json` | 机器可读数据清单 |
| `reevaluate_counts_predictions_log1p.py` | counts→log1p 重新评估脚本 |
| `README.md` | 本文件 |
