...
Usage:
  ...
  python scripts/evaluate_nips_gbm.py --predicted GBM_CPA_PW034_Panobinostat_pred.h5ad --method CPA_M0
"""
...
def evaluate_predictions(adata_path, pred_path, method, setting, args):
    ...
    # Get DEGs
    deg_dict = adata.uns.get("top50_DEGs", {})
    ...
    for group in cov_drug_groups:
        ...
        treated_mask_np = eval_split_mask_np & (adata.obs["cov_drug_name"].astype(str) == str(group)).to_numpy() & (~adata.obs["is_control"]).to_numpy()
        # Control cells: same cell_type, is_control. Try evaluation split first, then any split
        ctrl_mask_np = eval_split_mask_np & (adata.obs["cell_type"].astype(str) == cell_type_str).to_numpy() & adata.obs["is_control"].to_numpy()
        if ctrl_mask_np.sum() < args.min_ctrl:
            ctrl_mask_np = (adata.obs["cell_type"].astype(str) == cell_type_str).to_numpy() & adata.obs["is_control"].to_numpy()
        ...
        # For CPA predictions: match by cell_type and perturbation columns
        if "cell_type" in pred.obs.columns and "perturbation" in pred.obs.columns:
