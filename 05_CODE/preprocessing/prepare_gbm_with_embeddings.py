...
前置步骤（在 nature 环境中运行）：
  1. python 05_CODE/preprocessing/encode_gbm_cells_scgpt.py
  2. python 05_CODE/preprocessing/encode_gbm_drugs_molformer.py

用法：
  conda activate plknature
  python 05_CODE/preprocessing/prepare_gbm_with_embeddings.py \
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--adata",
        type=Path,
        default=REUSABLE / "preprocessed_data" / "GBM_Universal_Perturbation_Ready.h5ad",
        help="Original GBM h5ad (read-only, never modified).",
    )
    parser.add_argument(
        "--scgpt",
        type=Path,
        default=REUSABLE / "preprocessed_data" / "GBM_scGPT_embeddings.h5ad",
        help="scGPT output h5ad containing obsm X_scGPT, X_scGPT_ctrl, X_scGPT_pert.",
    )
    parser.add_argument(
        "--molformer",
        type=Path,
        default=REUSABLE / "preprocessed_data" / "GBM_X_MolFormer.npy",
        help="MolFormer per-cell embeddings (npy).",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=REUSABLE / "preprocessed_data" / "GBM_with_embeddings.h5ad",
        help="Output h5ad with all embeddings in obsm.",
    )
    parser.add_argument(
        "--drug-parquet",
        type=Path,
        default=REUSABLE / "embeddings" / "GBM_molformer_drug_emb.parquet",
