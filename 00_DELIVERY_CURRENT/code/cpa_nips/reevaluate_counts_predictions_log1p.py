...
DELIVERY = os.path.join(ROOT, "00_DELIVERY_CURRENT")
RUNTIME = os.path.join(ROOT, "02_RUNTIME_RESULTS")
...
def main():
    ...
    parser.add_argument("--output-dir", default=os.path.join(DELIVERY, "evaluation", "unified_log1p"),
    ...
    if h5ad_path is None:
        for p in [os.path.join(DELIVERY, "dataset", "GBM_NIPS_Ready.h5ad"), "GBM_NIPS_Ready.h5ad"]:
            if os.path.exists(p):
                h5ad_path = p
                break
    ...
    pred_files = {
        "CPA_M0": {"file": os.path.join(RUNTIME, "predictions", "legacy_comparison", "GBM_CPA_PW034_Panobinostat_pred.h5ad"), "space": "counts"},
        "CPA_M4_MolFormer": {"file": os.path.join(RUNTIME, "predictions", "legacy_comparison", "GBM_CPA_MolFormer_PW034_Panobinostat_pred.h5ad"), "space": "counts"},
        "CPA_M1_scGPT": {"file": os.path.join(RUNTIME, "predictions", "legacy_comparison", "GBM_CPA_scGPT_PW034_Panobinostat_pred.h5ad"), "space": "log1p"},
        "CPA_M2_scGPT_ctrl": {"file": os.path.join(RUNTIME, "predictions", "legacy_comparison", "GBM_CPA_scGPT_ctrl_PW034_Panobinostat_pred.h5ad"), "space": "log1p"},
        "CPA_M3_scGPT_pert": {"file": os.path.join(RUNTIME, "predictions", "legacy_comparison", "GBM_CPA_scGPT_pert_PW034_Panobinostat_pred.h5ad"), "space": "log1p"},
        "CPA_M5_scGPT_MolFormer": {"file": os.path.join(RUNTIME, "predictions", "legacy_comparison", "GBM_CPA_scGPT_MolFormer_PW034_Panobinostat_pred.h5ad"), "space": "log1p"},
        "MeanShiftBaseline": {"file": os.path.join(RUNTIME, "predictions", "legacy_comparison", "GBM_MeanShiftBaseline_PW034_Panobinostat_pred.h5ad"), "space": "log1p"},
    }
