...
用 `00_DELIVERY_CURRENT/code/cpa_nips/reevaluate_counts_predictions_log1p.py` 可对 M0/M4 重新评估。
...
|--------|---------|---------|---------|------|
| M0 CPA | layers["counts"] | learnable emb | `00_DELIVERY_CURRENT/dataset/GBM_NIPS_Ready.h5ad` | ✅ |
...
存储于 `02_RUNTIME_RESULTS/evaluation/legacy_nips/`，包含 9 个 JSON 文件和 CRISP OOD metrics markdown。这些指标的预测空间和协议与当前统一标准可能存在差异。

### 15.2 统一 Log1p Metrics

所有方法统一在 log1p space 下重新评估，M0/M4 的 counts 预测已通过 `np.log1p` 转换。结果存储于 `00_DELIVERY_CURRENT/evaluation/unified_log1p/`。
